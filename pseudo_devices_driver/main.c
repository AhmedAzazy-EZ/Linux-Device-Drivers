...
{
	struct pcd_dev_data * pcd_prv_data;
	printk("open function...retreiving private data\n");
	
	pcd_prv_data = container_of(inode->i_cdev, struct pcd_dev_data , pcd_cdev);
	filp->private_data = pcd_prv_data;
	printk("Detected Serial: %s\n" , pcd_prv_data->Dev_Serial);
...
{
	struct pcd_dev_data * pcd_prv_data;
	pcd_prv_data = filp->private_data;
	printk("%s\n" , pcd_prv_data->buff);
	printk("Read Done...\n");
	copy_to_user(user_buff , pcd_prv_data->buff , count);
	return strlen(pcd_prv_data->buff);
/*
#if DEBUG==1
	return 0;
#endif
...
ssize_t pcd_write (struct file * filp, const char __user * user_buff , size_t count, loff_t * f_pos)
{
	struct pcd_dev_data * pcd_prv_data;
	pcd_prv_data = filp->private_data;
	if(strlen(user_buff) > pcd_prv_data->size)
		{
			printk("size is too large\n");
			return 0;
		}
	
	else 
	{
		if(copy_from_user(Dev1_buff , user_buff , strlen(user_buff)+1) != 0 )
			{
			
				printk("Error has happened while copying from user\n");
			}
		...
		else 
		{
			...
			printk("Copy is Done \n");
			printk("the length of written string is %ld\n" , strlen(user_buff));
		}
		
	}
	
	return strlen(user_buff);
/*	
#if DEBUG==1
	return 0;
#endif
...
static int __init pcd_driver_init(void)
{
	...
	cd = alloc_chrdev_region ( &My_driver.dev , 0 , NUM_OF_DEVICES , "pcd_driver" );
	if (cd < 0)
		goto alloc_failed;
